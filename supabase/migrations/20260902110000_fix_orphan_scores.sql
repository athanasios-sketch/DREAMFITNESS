...
-- A day with no day_log is an UNLOGGED day, not a zero-scoring one.
-- Without this, deleting a log leaves a 0-score row behind and the dashboard
-- counts it as logged.
create or replace function public.score_day(p_date date)
returns numeric language plpgsql security invoker set search_path = '' as $$
declare v_user uuid := (select auth.uid()); v_has boolean;
begin
  if v_user is null then raise exception 'not authenticated'; end if;
  select exists (select 1 from public.day_logs where user_id = v_user and log_date = p_date)
    into v_has;
  if not v_has then
    delete from public.daily_scores where user_id = v_user and score_date = p_date;
    return null;
  end if;
  return public.score_day_inner(p_date);
end $$;
